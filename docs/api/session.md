...
* `handler` Function | null
  * `webContents` ([WebContents](web-contents.md) | null) - WebContents requesting the permission. Please note that if the request comes from a subframe you should use `requestingUrl` to check the request origin. Platform permission requests, such as `local-network`, may pass `null`.
...
session.fromPartition('some-partition').setPermissionRequestHandler((webContents, permission, callback) => {
  if (webContents && webContents.getURL() === 'some-host' && permission === 'notifications') {
